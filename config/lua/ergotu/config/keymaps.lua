...
map("n", "<C-S-Left>", "<cmd>vertical resize -2<cr>", { desc = "Decrease Window Width", remap = true })
map("n", "<C-S-Right>", "<cmd>vertical resize +2<cr>", { desc = "Increase Window Width", remap = true })

map("n", "<C-S-k>", "<cmd>resize +2<cr>", { desc = "Increase Window Height", remap = true })
map("n", "<C-S-j>", "<cmd>resize -2<cr>", { desc = "Decrease Window Height", remap = true })
map("n", "<C-S-h>", "<cmd>vertical resize -2<cr>", { desc = "Decrease Window Width", remap = true })
map("n", "<C-S-l>", "<cmd>vertical resize +2<cr>", { desc = "Increase Window Width", remap = true })
...
local function diagnostic_goto(next, severity)
  return function()
    local go = function(opts)
      vim.diagnostic.jump(vim.tbl_extend("force", { count = next and 1 or -1 }, opts or {}))
    end
    local sev = severity and vim.diagnostic.severity[severity] or nil
    go({ severity = sev, float = false })
  end
end
...
map("n", "<leader>ft", function()
  Snacks.terminal(nil, { cwd = Ergovim.root.get() })
